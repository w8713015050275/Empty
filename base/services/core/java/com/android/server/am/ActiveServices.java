...
public final class ActiveServices {
    ...
    static boolean DEBUG_DELAYED_SERVICE = DEBUG_SERVICE;
    ...
    class ServiceMap extends Handler {
        ...
        final ArrayMap<ComponentName, ServiceRecord> mShadowServicesByName
                = new ArrayMap<ComponentName, ServiceRecord>();
        final ArrayMap<Intent.FilterComparison, ServiceRecord> mShadowServicesByIntent
        = new ArrayMap<Intent.FilterComparison, ServiceRecord>();
        //[+LEUI]end
    ...
    ServiceRecord getServiceByName(ComponentName name, int callingUser) {
	 ...
	 //[+LEUI] [REQ][LEUI-8957] [xulei] add 
        ...
        boolean isClone = mAm.isCloneProcessByPid(pid);
        if(isClone && name != null && name.getPackageName() != null){
            try {
                    isDouble = AppGlobals.getPackageManager().isAppDouble(name.getPackageName());
        ...
        if(isClone && isDouble){
            return getServiceMap(callingUser).mShadowServicesByName.get(name);
        }else{
        //[+ELUI]end
    ...
    ArrayMap<ComponentName, ServiceRecord> getServices(int callingUser) {
        return getServiceMap(callingUser).mServicesByName;
         ...
         //[+LEUI] [REQ][LEUI-8957] [xulei] add 
         //for support double app
    ArrayMap<ComponentName, ServiceRecord> getShadowServices(int callingUser) {
    ...
    ComponentName startServiceLocked(IApplicationThread caller, Intent service, String resolvedType,
        ...
        if (caller != null) {
           ...
           //[+LEUI] [REQ][LEUI-8957] [xulei] add 
           //for support double app
            if(callerApp.isShadow)
            {
                service.addCategory(ActivityManagerService.LETV_CLONE);
                if(DEBUG_DOUBLE_APP)
                Slog.d(TAG, "Double app  startServiceLocked  clone service:" + service+", callApp:"+callerApp);
            }
            //[+LEUI]end
        } else {
            callerFg = true;
        ...
        //[-LEUI-18511]
        service.setCallingUid(callingUid);
        //[-LEUI-18511]
        ...
        if (!callerFg && r.app == null && mAm.mStartedUsers.get(r.userId) != null) {
            ProcessRecord proc = null;
             ...
             //for support double app
            if(r.isShadow){
                proc = mAm.getProcessRecordLocked(r.processName, r.appInfo.uid+r.appInfo.uidOffset, false);
                if(DEBUG_DOUBLE_APP)
                Slog.v(TAG, "double app getProcess proc:" + proc+", prcessName:"+r.processName);
            }else{
            //[+LEUI]end
                proc = mAm.getProcessRecordLocked(r.processName, r.appInfo.uid, false);
            }
    ...
    public void setServiceForegroundLocked(ComponentName className, IBinder token,
        ...
        try {
            ...
            if (r != null) {
                if (id != 0) {
                    //modify by letv for give system app foreground adj without notification
                    if(id == -255 && (r.appInfo.flags & ApplicationInfo.FLAG_SYSTEM)!=0){
                        r.isForeground = true;
                    } else {
    ...
    int bindServiceLocked(IApplicationThread caller, IBinder token, Intent service,
        ...
        if (callerApp == null) {
         ...
         //[+LEUI] [REQ][LEUI-8957] [xulei] add 
         //for support double app
        if(callerApp != null && callerApp.isShadow)
        {
            service.addCategory(ActivityManagerService.LETV_CLONE);
            if(DEBUG_DOUBLE_APP)
            Slog.d(TAG, "double app  bindServiceLocked service:" + service+", callApp:"+callerApp);
        }
        //[+LEUI]end
    ...
    private ServiceLookupResult retrieveServiceLocked(Intent service,
        ...
        if(service.hasCategory(ActivityManagerService.LETV_CLONE) && comp != null){
            String pkg = comp.getPackageName();
             try {
                    boolean isDouble = AppGlobals.getPackageManager().isAppDouble(pkg);
                    if(isDouble){
                        isShadow = true;
                    }
                    if(DEBUG_DOUBLE_APP)
                    Slog.v(TAG, "double app. retrieveServiceLocked Provider isDouble;"+isDouble+", pkg;"+pkg+", service:"+service);
            } catch (RemoteException ex) {
                // pm is in same process, this will never happen.
            }
        }else if(service.hasCategory(ActivityManagerService.LETV_CLONE) && comp == null){
            Slog.w(TAG, "double app. retrieveServiceLocked service intent hasCategory, but comp is null!!!"+", callingPid;"+callingPid+", service:"+service);
        }
                    
        if(isShadow){
            if (comp != null) {
                r = smap.mShadowServicesByName.get(comp);
            }
            if (r == null) {
                Intent.FilterComparison filter = new Intent.FilterComparison(service);
                r = smap.mShadowServicesByIntent.get(filter);
            }
        }else{
        //[+LEUI] end
             if (comp != null) {
        ...
        if (r == null) {
            try {
                ...
                // [+LEUI] [REQ] [LEUI-23808] [yuanguohong] limit apps to launch automatically.
                if(!mAm.mAppAutoLaunchManager.isAllowLaunch(sInfo, service, callingUid, callingPid, userId)) {
                    Slog.w(TAG, "don't allow to launch " + sInfo);
                    return null;
                }
                // [-LEUI]
                if (userId > 0) {
                ...
                //for support double app
                if(isShadow){
                    r = smap.mShadowServicesByName.get(name);
                }else{
                //[+LEUI] end
                ...
                if (r == null && createIfNeeded) {
                    ...
                    if(isShadow){
                        if(DEBUG_DOUBLE_APP)
                        Slog.w(TAG, "double app retrieveServiceLocked put clone servicesMap sInfo.name: " + sInfo.name + " .r=" + r);
                        smap.mShadowServicesByName.put(name, r);
                        smap.mShadowServicesByIntent.put(filter, r);
                    }else{
                    //[+LEUI]end
                    ...
                    for (int i=mPendingServices.size()-1; i>=0; i--) {
                        ...
                        //for support double app
                        if(pr.isShadow){
                            if ((pr.serviceInfo.applicationInfo.uid + pr.serviceInfo.applicationInfo.uidOffset) == (sInfo.applicationInfo.uid +  sInfo.applicationInfo.uidOffset)
                                    && pr.name.equals(name)) {
        ...
        if (r != null) {
            ...
            // [+LEUI] [REQ] [LEUI-23808] [yuanguohong] limit apps to launch automatically.
            if(r.serviceInfo != null && !mAm.mAppAutoLaunchManager.isAllowLaunch(
                    r.serviceInfo, service, callingUid, callingPid, userId)) {
                Slog.w(TAG, "don't allow to launch " + r.serviceInfo);
                return null;
            }
            // [-LEUI]
    ...
    private final boolean scheduleServiceRestartLocked(ServiceRecord r,
        ...
        //for support double app
        if(r.isShadow){
            if (smap.mShadowServicesByName.get(r.name) != r) {
                ServiceRecord cur = smap.mShadowServicesByName.get(r.name);
    ...
    private final String bringUpServiceLocked(ServiceRecord r, int intentFlags, boolean execInFg,
        ...
        if (!isolated) {
         ...
         //for support double app
            if(r.isShadow){
                app = mAm.getProcessRecordLocked(procName, r.appInfo.uid+ r.appInfo.uidOffset, false);
                if(DEBUG_DOUBLE_APP)
                Slog.v(TAG, "double app getProcess app:" + app+", procName:"+procName);
            }
            else{
                //[+LEUI]end
    ...
    private final void sendServiceArgsLocked(ServiceRecord r, boolean execInFg,
        ...
        while (r.pendingStarts.size() > 0) {
            ...
            try {
                ...
                //[+LEUI-24824] checktime
                if(si.intent!=null){
                    si.intent.setArg1(SystemClock.uptimeMillis());
    ...
    private final void bringDownServiceLocked(ServiceRecord r) {
        ...
        final ServiceMap smap = getServiceMap(r.userId);
         ...
         //for support double app
        if(r.isShadow){
             smap.mShadowServicesByName.remove(r.name);
            smap.mShadowServicesByIntent.remove(r.intent);            
        }else{
        //[+LEUI]end
        ...
        if (r.executeNesting <= 0) {
            if (r.app != null) {
                ...
                if (r.app.executingServices.size() == 0) {
                    ...
                    LetvDebugManager.getInstance().removeEvent(
                                    LetvDebugManager.START_MONITOR_SERVICE_TIMEOUT_MSG,r.app);
                } else if (r.executeFg) {
            ...
            try {
                for (int i=0; i<mPendingServices.size(); i++) {
                    ...
                    if (proc != sr.isolatedProc && (proc.uid != sr.appInfo.uid
                     ...
                     //[+LEUI] [REQ][LEUI-8957] [xulei] add 
                     //for support double app
                    if(sr.isShadow != proc.isShadow){
                        Slog.w(TAG, "xulei check attachApplicationLocked proc: " + proc + " rs=" + sr);  
                        continue;
                    }
                    //[+LEUI]end
                    mPendingServices.remove(i);
        ...
        if (mRestartingServices.size() > 0) {
            ...
            for (int i=0; i<mRestartingServices.size(); i++) {
                ...
                if (proc != sr.isolatedProc && (proc.uid != sr.appInfo.uid
                 ...
                 //for support double app
                 if(sr.isShadow != proc.isShadow){
                    Slog.w(TAG, "xulei check attachApplicationLocked 123 proc: " + proc + " rs=" + sr);  
                    continue;
                }
                //[+LEUI]end
        ...
        for (int i=0; i<mPendingServices.size(); i++) {
            ...
            if ((proc.uid == sr.appInfo.uid
                ...
                Slog.w(TAG, "Forcing bringing down service: " + sr);
                 ...
                 //for support double app
                 if(sr.isShadow != proc.isShadow){
                    Slog.w(TAG, "xulei check processStartTimedOutLocked 123 proc: " + proc + " rs=" + sr);  
                    continue;
                }
                //[+LEUI]end
    ...
    //[+LEUI] [REQ][LEUI-8957] [xulei] add 
    //for support double app  
    boolean forceStopLocked(String name, Set<String> filterByClasses, int userId, boolean evenPersistent, boolean killProcess, boolean doit, boolean isShadow) {
        boolean didSomething = false;
        ...
        if (userId == UserHandle.USER_ALL) {
            for (int i=0; i<mServiceMap.size(); i++) {
                ...
                if(isShadow){
                    items = mServiceMap.valueAt(i).mShadowServicesByName;
                }else{
                    items = mServiceMap.valueAt(i).mServicesByName;
                }
               didSomething |= collectPackageServicesLocked(name, filterByClasses, evenPersistent,
                doit, killProcess,items);
            ...
            if (smap != null) {
                ArrayMap<ComponentName, ServiceRecord> items;
                if(isShadow){
                    items = smap.mShadowServicesByName;
               }else{
                    items = smap.mServicesByName;
               }
                didSomething = collectPackageServicesLocked(name, filterByClasses, evenPersistent,
                doit, killProcess, items);
            }
    ...
    void cleanUpRemovedTaskLocked(TaskRecord tr, ComponentName component, Intent baseIntent) {
        ...
        ArrayMap<ComponentName, ServiceRecord> alls = null;
         //[+LEUI] [REQ][LEUI-8957] [xulei] add 
         //for support double app
        if(tr.isShadow){
            alls = getShadowServices(tr.userId);
        }else{
        //[+LEUI]end
            alls = getServices(tr.userId);
        }
        //[+LEUI] [REQ][RUBY-18121] [yueshuai] add
        if(tr.isAccessControlTask()) return ;
        //[+LEUI]end
    ...
    final void killServicesLocked(ProcessRecord app, boolean allowRestart) {
        ...
        for (int i=app.services.size()-1; i>=0; i--) {
			...
			//[+LEUI] [REQ][LEUI-8957] [xulei] add 
            //for support double app  
            final ServiceRecord curRec;
            if(sr.isShadow){
                curRec = smap.mShadowServicesByName.get(sr.name);
            }else{
                curRec = smap.mServicesByName.get(sr.name);
            }
			//[+LEUI]end
            if (curRec != sr) {
                if (curRec != null) {
        ...
        if (!allowRestart) {
            ...
            for (int i=mRestartingServices.size()-1; i>=0; i--) {
                ...
                if (r.processName.equals(app.processName) &&
                    ...
                    //[+LEUI] [REQ][LEUI-8957] [xulei] add 
                    //for support double app  
                    if(r.isShadow != app.isShadow){
                        Slog.w(TAG, "xulei check killServicesLocked  proc: " + app + " r=" + r);  
                        continue;
                    }
                    //[+LEUI]end
            ...
            for (int i=mPendingServices.size()-1; i>=0; i--) {
                ...
                if (r.processName.equals(app.processName) &&
              	...
              	//[+LEUI] [REQ][LEUI-8957] [xulei] add 
                    //for support double app  
                     if(r.isShadow != app.isShadow){
                        Slog.w(TAG, "xulei check killServicesLocked 123 app: " + app + " r=" + r);  
                        continue;
                    }
                     //[+LEUI]
                    mPendingServices.remove(i);
    ...
    List<ActivityManager.RunningServiceInfo> getRunningServiceInfoLocked(int maxNum,
        ...
        try {
            if (ActivityManager.checkUidPermission(
		        ...
		        //for support double app  
                for (int ui=0; ui<users.length && res.size() < maxNum; ui++) {
                    ArrayMap<ComponentName, ServiceRecord> alls = getShadowServices(users[ui]);
                    for (int i=0; i<alls.size() && res.size() < maxNum; i++) {
                        ServiceRecord sr = alls.valueAt(i);
                        res.add(makeRunningServiceInfoLocked(sr));
                    }
    ...
    void serviceTimeout(ProcessRecord proc) {
        ...
        synchronized(mAm) {
            ...
            if (timeout != null && mAm.mLruProcesses.contains(proc)) {
                ...
                //predump for timeout
                if(!proc.notResponding){//if already anr process , ignore
                    Slog.w(TAG, "LetvDebugManager resend: " + proc);
                    long timeoutMonitor =proc.execServicesFg ? (nextTime+SERVICE_TIMEOUT*3/4) : (nextTime+ SERVICE_BACKGROUND_TIMEOUT*3/4);
                    LetvDebugManager.getInstance().sendEventAtTime(
                                  LetvDebugManager.START_MONITOR_SERVICE_TIMEOUT_MSG,proc.pid,timeoutMonitor,proc);
                }
            }
        }
    ...
    void scheduleServiceTimeoutLocked(ProcessRecord proc) {
        ...
        //predump for timeout
        long timeoutMonitor =proc.execServicesFg ? (now+SERVICE_TIMEOUT*3/4) : (now+ SERVICE_BACKGROUND_TIMEOUT*3/4);
        LetvDebugManager.getInstance().sendEventAtTime(
                        LetvDebugManager.START_MONITOR_SERVICE_TIMEOUT_MSG,proc.pid,timeoutMonitor,proc);
    }
        ...
        try {
            ...
            for (int user : users) {
                ...
                if (smap.mServicesByName.size() > 0) {
                    ...
                    //[+LEUI] [REQ][LEUI-8957] [xulei] add 
                    //for support double app
                    for (int si=0; si<smap.mShadowServicesByName.size(); si++){
                        ServiceRecord r = smap.mShadowServicesByName.valueAt(si);
    ...
    void enableDebug(boolean enable) {
        DEBUG_DELAYED_SERVICE = enable;
        DEBUG_DELAYED_STARTS = enable;
    }
}
