...
import android.content.pm.ILaunchObserver;
// [-LEUI]
/**
...
interface IPackageManager {
    ...
    //定义跨进程接口的aidl文件
    ResolveInfo getCloneResolveInfo(in ResolveInfo rInfo);
    boolean isAppDouble(String packageName);
    //[+LEUI]end
    // [+LEUI] [REQ] [LEUI-13225] [xulei] added:feature for limit background apps.
    boolean  isSameSignsApps(String pkg1, String pkg2);
	...
	boolean copyMMFile(int uid, in String seinfo, in String dest_path, in String reserve1, in String reserve2, int userId);
   ...
   /**
    ...
    void recoverySystemInstallPackage(String packagepath);
    // [-LEUI]
    // [+LEUI] [REQ][LEUI-6231] [zhaowanxiang] added
    // function: When adb push a system apk, which can be installed
    void pmScanPackageLI(String scanFile, int parseFlags, int scanFlags,long currentTime);
    //[-LEUI]
    // [+LEUI] [REQ][LEUI-5820] [zhaowanxiang] added for have callback interface for user get this pacakge be launched
    void registerLaunchedObserver(ILaunchObserver launchObserver);

    void unRegisterLaunchedObserver();

    void setLaunchedPackage(in Intent intent, String packageName);
    //[-LEUI]
    ...
    //[+LEUI-9268]
    ResolveInfo getAccessCtrlResolveInfo(int userId);
    //[-LEUI-9268]
    // [+LEUI] [REQ][LEUI-13264] [luochen1] add for disable app function in SuperManager
    String getLastDisabledAppCaller(String packageName, int userId);
    // [-LEUI]

    // [+LEUI] [REQ][LEUI-12173] [zhaowanxiang] add 2015.1.26
    // function: Get the state of installing package
    boolean getInstallingPackageState(String PackageName);
    // [-LEUI]

    // [+LEUI] [REQ][LEUI-25003] [zhangfeng3] add 2016.11.08
    // function: Grant permission to application itself
    void grantSelfRuntimePermission(String packageName, String permissionName, int userId);
    // [-LEUI]
}
