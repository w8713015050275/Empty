 ...
 */
//fragment 后退栈的入口
    //FragmentManager.beginTransaction()中创建事务的子类对象 BackStackRecord
final class BackStackRecord extends FragmentTransaction implements
    ...
    //FragmentTransaction.beginTransaction(): 创建BackStackRecord对象
    ...
    void addOp(Op op) {
        ...
        //fragment的动画,在setCustomAnimations()中去自定义
    ...
    /**
     *处理FragmentTransaction提交的操作Op
     * @param allowStateLoss ture表示允许在onSaveInstanceState()后提交transaction
     * @return
     */
    int commitInternal(boolean allowStateLoss) {
        ...
        //入队当前事务,BackStackRecord,继承了Runnable
        mManager.enqueueAction(this, allowStateLoss);
        return mIndex;
    }

    //实现了Runnable接口,通过主线程的handler 调用execPendingActions(), 执行到这里
    public void run() {
        if (FragmentManagerImpl.DEBUG) {
            Log.v(TAG, "Run: " + this);
        }
        ...
        while (op != null) {
            ...
            switch (op.cmd) {
                ...
                case OP_REPLACE: {
                    ...
                    //应该是不为null的, 添加状态的fragment
                    if (mManager.mAdded != null) {
                        for (int i = 0; i < mManager.mAdded.size(); i++) {
                            //从added状态的数组中获得一个fragment
                            Fragment old = mManager.mAdded.get(i);
                            if (FragmentManagerImpl.DEBUG) {
                                Log.v(TAG,
                                        "OP_REPLACE: adding=" + f + " old=" + old);
                            }
                            //如果得到的fragment的view id 与将要添加的容器 id相等
