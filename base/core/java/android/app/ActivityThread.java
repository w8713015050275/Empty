...
//chenzhiyong RUBY-12498 start
import java.security.Security;
//chenzhiyong RUBY-12498 end
...
public final class ActivityThread {
    ...
    //[+LEUI-8930]
    private static final String LEUI_TAG = "ActivityThreadEui";
    private static final boolean ENABLE_LEUI_MECHANISM = true;
    //[-LEUI-8930]
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public final void schedulePauseActivity(IBinder token, boolean finished,
            ...
            if(r != null && r.activity != null){
                Slog.i(LEUI_TAG, "schedulePauseActivity "+r.activity.getComponentName().getClassName()
                        +" finished="+finished+" userLeaving="+userLeaving
                        +" configChanges="+configChanges+" dontReport="+dontReport);
            }
            //[-LEUI-15516]
            sendMessage(
        ...
        public final void scheduleStopActivity(IBinder token, boolean showWindow,
            ...
            ActivityClientRecord r = mActivities.get(token);
            if(r != null && r.activity != null){
                Slog.i(LEUI_TAG, "scheduleStopActivity "+r.activity.getComponentName().getClassName()
                        +" show="+showWindow+" configChanges="+configChanges);
            }
            //[-LEUI-15516]
           sendMessage(
        ...
        public final void scheduleWindowVisibility(IBinder token, boolean showWindow) {
            ...
            ActivityClientRecord r = mActivities.get(token);
            if(r != null && r.activity != null){
                Slog.i(LEUI_TAG, "scheduleWindowVisibility "+r.activity.getComponentName().getClassName()
                        +" show="+showWindow);
            }
            //[-LEUI-15516]
            sendMessage(
        ...
        public final void scheduleSleeping(IBinder token, boolean sleeping, boolean screenOn/*LEUI-10257*/) {
            sendMessage(H.SLEEPING, token, sleeping ? 1 : 0, screenOn ? 1 : 0);
        }

        public final void scheduleResumeActivity(IBinder token, int processState,
                boolean isForward, int reasonFlags/*LEUI-10257*/, Bundle resumeArgs) {
            //[+LEUI-15516]
            ActivityClientRecord r = mActivities.get(token);
            if(r != null && r.activity != null){
                Slog.i(LEUI_TAG, "scheduleResumeActivity "+r.activity.getComponentName().getClassName());
            }
            //[-LEUI-15516]
            updateProcessState(processState, false);
            sendMessage(H.RESUME_ACTIVITY, token, isForward ? 1 : 0, reasonFlags);
        }
        ...
        public void enableActivityThreadLog(boolean enable, String flags) {
            ActivityThread.this.enableActivityThreadLog(enable, flags);
        }
    }
            ...
            if (DEBUG_MESSAGES) {
                switch (code) {
                    ...
                    case ENTER_ANIMATION_COMPLETE: return "ENTER_ANIMATION_COMPLETE";
                        //[+LEUI] [TASK][RUBY-20880] [xulei] add 
                    case START_TRACEVIEW_MSG: return "START_TRACEVIEW_MSG";
                    //end
                }
            }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case RESUME_ACTIVITY:
                    ...
                    handleResumeActivity((IBinder) msg.obj, true, msg.arg1 != 0,msg.arg2/*LEUI-10257*/, true);
                    Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
                    break;
                case SEND_RESULT:
                ...
                case SLEEPING:
                    ...
                    handleSleeping((IBinder)msg.obj, msg.arg1 != 0, msg.arg2 != 0);
                    Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
                    break;
                ...
                case ENTER_ANIMATION_COMPLETE:
                    ...
                    break;
                 //[+LEUI] [TASK][RUBY-20880] [xulei] add 
                case START_TRACEVIEW_MSG:
                    processTraceViewMsg(msg);
                    break;
                    //end
            }
    ...
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        if (r.packageInfo == null) {
            //从ActivityInfo中获得activity对应的apk info：LoadedApk
            r.packageInfo = getPackageInfo(aInfo.applicationInfo, r.compatInfo,
                    Context.CONTEXT_INCLUDE_CODE);
        }

        //如果显示调用：activity对应的component，根据此ComponentName去创建Activity对象
        ComponentName component = r.intent.getComponent();
        if (component == null) {
            //隐式调用：去解析Intent，得到ComponentName
            component = r.intent.resolveActivity(
                mInitialApplication.getPackageManager());
            //将ComponentName赋值给Intent对象
            r.intent.setComponent(component);
        }

        if (r.activityInfo.targetActivity != null) {
            //通过Activity的string name,栗子"CustomActivity" 创建ComponentName
        ...
        try {
            ...
            if (activity != null) {
                //创建与LoadedApk关联的ContextImpl
                Context appContext = createBaseContextForActivity(r, activity);
                CharSequence title = r.activityInfo.loadLabel(appContext.getPackageManager());
                Configuration config = new Configuration(mCompatConfiguration);
                if (DEBUG_CONFIGURATION) Slog.v(TAG, "Launching activity "
                        + r.activityInfo.name + " with config " + config);
                //attach中：对appContext的操作，就是将appContext赋值给Activity的父父类ContextWrapper的
                //mBase变量；所以可以再Activity中操作ContextImpl的方法；
                ...
                if (r.isPersistable()) {
                    ...
                    //回调到onCreate();这些生命周期方法的回调都是通过mInstrumentation来调用；
                ...
                //把创建好的Activity对象给r：ActivityClientRecord
                r.activity = activity;
                r.stopped = true;
    ...
    private Context createBaseContextForActivity(ActivityClientRecord r, final Activity activity) {
        ...
        //通过该ActivityRecordClient对象的实例的packageInfo：LoadedApk构造appContext，说明Context是与apk资源
        //关联，并非跟Activity关联；但是每个Activity实例都会去创建一个ContextImpl
        ContextImpl appContext = ContextImpl.createActivityContext(
                this, r.packageInfo, displayId, r.overrideConfig);
        //指定该ContextImpl是由哪个Activity初始化的时候创建的
        ...
        //返回一个Activity初始化时创建的与LoadedApk关联的ContextImpl对象
        return baseContext;
    }
    ...
    private void handleLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        //[+LEUI-15516]
        Slog.i(LEUI_TAG,"Handling launch of " + r +  " startsNotResumed=" + r.startsNotResumed);
        //[-LEUI-15516]
        ...
        if (a != null) {
            ...
            handleResumeActivity(r.token, false, r.isForward,r.reasonFlags/*LEUI-10257*/,
    ...
    private void deliverNewIntents(ActivityClientRecord r, List<ReferrerIntent> intents) {
        ...
        //[+LEUI-10257]
        Log.i(LEUI_TAG, "deliverNewIntents resonFlags="+Integer.toHexString(reasonFlags));
    ...
    //[+LEUI-10257]
    private void checkReasonFlags(ActivityClientRecord r){
        if(ENABLE_LEUI_MECHANISM &&
                r != null && r.activity != null && r.activity.getWindow() != null){
            final int reasonFlags = r.reasonFlags;
            final Window win = r.activity.getWindow();
            Log.d(LEUI_TAG,"check reason flags for activity window ="
                    +r.activityInfo.name+" reasonFlags="+Integer.toHexString(reasonFlags));
            if(win != null && (reasonFlags & Intent.FLAG_SHOW_WHEN_LOCKED) != 0){
                WindowManager.LayoutParams lp = win.getAttributes();
                if((lp.flags & WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED) ==0 ){
                    lp.flags |= WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED;
                    View decor = win.getDecorView();
                    if(decor.getParent() != null && r.activity.mWindowAdded){
                        final ViewManager wm = r.activity.getWindowManager();
                        Log.d(LEUI_TAG,"need updateViewlayout add flags");
    ...
    private void clearWindowFlags(ActivityClientRecord r){
        if(ENABLE_LEUI_MECHANISM &&
                r != null && r.activity != null && r.activity.getWindow() != null){
            final int reasonFlags = r.reasonFlags;
            final Window win = r.activity.getWindow();
            Log.d(LEUI_TAG,"clear window flag(FLAG_SHOW_WHEN_LOCKED)for activity window ="
                    +r.activityInfo.name+"flags="+Integer.toHexString(reasonFlags));
            if((reasonFlags & Intent.FLAG_SHOW_WHEN_LOCKED) != 0 && win != null){
                r.reasonFlags &= ~Intent.FLAG_SHOW_WHEN_LOCKED;
                WindowManager.LayoutParams lp = win.getAttributes();
                if((lp.flags & WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED) !=0){
                    lp.flags &= ~WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED;
                    View decor = win.getDecorView();
                    if(decor.getParent() != null && r.activity.mWindowAdded){
                        final ViewManager wm = r.activity.getWindowManager();
                        Log.d(LEUI_TAG,"need updateViewlayout clear flag");
                        wm.updateViewLayout(decor, lp);
                    }
                }
            }
    ...
    private void performStopActivityInner(ActivityClientRecord r,
        ...
        if (r != null) {
            ...
            if (!keepShown) {
                ...
                //[+LEUI-10257]
                if(ENABLE_LEUI_MECHANISM &&
                        (r.reasonFlags & Intent.FLAG_SHOW_WHEN_LOCKED) != 0){
                    Log.d(LEUI_TAG, "clear window flag for activty window "+r.activityInfo.name
                            +" because activity stopped");
                    //For security. remove the task that is associated to this activity.
                    //r.activity.finishAndRemoveTask();
                    clearWindowFlags(r);
                }
                //[-LEUI-10257]
                r.stopped = true;
            ...
            if (show) {
                if (!r.activity.mVisibleFromServer) {
                    ...
                    if (r.activity.mVisibleFromClient) {
                        ...
                        //See {@link Activity#setVisible}
                        Slog.i(LEUI_TAG, "updateVisibility "+r+" is NOT going to show a UI(makeVisible)");
                    }
                }
                if (r.newConfig != null) {
                    r.tmpConfig.setTo(r.newConfig);
    ...
    private void handleWindowVisibility(IBinder token, boolean show) {
        ...
        if (r.activity.mDecor != null) {
            ...
            //[+LEUI-15516]
            Slog.i(LEUI_TAG, "Handle window " + r + " visibility: " + show);
            //[-LEUI-15516]
            updateVisibility(r, show);
        }
        mSomeActivitiesChanged = true;
    ...
    private void handleSleeping(IBinder token, boolean sleeping, boolean screenOn/*LEUI-10257*/) {
        ActivityClientRecord r = mActivities.get(token);
        Log.i(LEUI_TAG,"handleSleeping sleeping="+sleeping+" screenOn="+screenOn);
        if (r == null) {
        ...
        if (sleeping) {
            ...
            //[+LEUI-10257]
            if(ENABLE_LEUI_MECHANISM &&
                    (r.reasonFlags & Intent.FLAG_SHOW_WHEN_LOCKED) != 0 && !screenOn){
                Log.d(LEUI_TAG, "clear window flag for activity window "+r.activityInfo.name
                        +" because activity sleeping and screen off");
                //For security. remove the task that is associated to this activity.
                //r.activity.finishAndRemoveTask();
                clearWindowFlags(r);
            }
    ...
    private void handleRelaunchActivity(ActivityClientRecord tmp) {
        ...
        //[+LEUI-15516]
        Slog.i(LEUI_TAG,"Handling relaunch of " + r);
        //[-LEUI-15516]
        if (r == null) {
            return;
        }
    ...
    final void handleConfigurationChanged(Configuration config, CompatibilityInfo compat) {
        ...
        //[+LEUI][dongshangyong]
        if (config.leGlobalFont > 0) {
            freeHardwareCacheIfNeeded(configDiff);
        }
        //[-LEUI]
    ...
    // [+LEUI][dongshangyong]
    static void freeHardwareCacheIfNeeded(int configDiff) {
        if (configDiff != 0) {
            boolean hasFontScaleConfigChange = ((configDiff & ActivityInfo.CONFIG_FONT_SCALE) != 0);
            if (hasFontScaleConfigChange) {
                Canvas.freeCaches();
                Canvas.freeTextLayoutCaches();
                android.graphics.Typeface.updateEUITypeface();
                WindowManagerGlobal.getInstance().freeHardwareCaches();
                if (DEBUG_CONFIGURATION) Slog.v(TAG, "Cleared Hardware Caches");
            }
        }
    }
    // [-LEUI]
    ...
    final void handleActivityConfigurationChanged(ActivityConfigChangeData data) {
        ...
        if (data.overrideConfig != null) {
            // overrideConfig should not contain changes of fontScale, otherwise
            // it might make Activity.onConfigurationChanged() not performed.
            data.overrideConfig.fontScale = 0;
            //[+LEUI][dongshangyong]
            data.overrideConfig.leFontScale = 0;
            //[-LEUI]
    ...
    private void handleBindApplication(AppBindData data) {
        ...
        if (data.instrumentationName != null) {
            ...
            try {
                ...
                //这里创建一个与apk 进程相关的Instrumentation对象
        ...
        if (!system) {
            ...
            try {
                ...
                ContextImpl context = ContextImpl.createAppContext(
                        this, getSystemContext().mPackageInfo);//android apk
                mInitialApplication = context.mPackageInfo.makeApplication(true, null); //key code
    ...
    public static ActivityThread systemMain() {
        ...
        thread.attach(true);//true表示为system, 创建了android application 和activityThread 绑定起来
        return thread;
    }
    ...
    public static void main(String[] args) {
        ...
        //[+LEUI][dongshangyong]
        // check for activity lunch and init eui typeface.
        android.graphics.Typeface.updateEUITypeface();
        //[-LEUI]
    ...
    private void enableActivityThreadLog(boolean enable, String flags) {
        Log.v(TAG, "enableActivityThreadLog: enable = " + enable + ", flags = " + flags);
        if (flags == null || flags.length() == 0)
            return;

        if(isHandleCurMessage(enable, flags))
            return;

        String[] flagArray = flags.split(",");
        if (flagArray[0].equals("a")) {
            localLOGV = enable;
            DEBUG_MESSAGES = enable;
            DEBUG_BROADCAST = enable;
            DEBUG_RESULTS = enable;
            DEBUG_BACKUP = enable;
            DEBUG_CONFIGURATION = enable;
            DEBUG_SERVICE = enable;
            DEBUG_MEMORY_TRIM = enable;
            DEBUG_PROVIDER = enable;
            return;
        }
        int count;
    ...
    //[+LEUI] [TASK][RUBY-20880] [xulei] add 
    public static final int START_TRACEVIEW_MSG     = 151;
    private String TRACEVIEW_CUR_FILE                   = null;

    private boolean isHandleCurMessage(boolean enable, String flags){
        String[] flagArray = flags.split(",");
        if(flagArray[0].equals("open_tr")){
            Log.w(TAG, "enable autoCatchTraceView catch 20*1000 ms , sava path:"+flags);
            autoCatchTraceView(1000*30, flags);
            return true;
        }else if(flagArray[0].equals("open_hprof")){
            Log.w(TAG, "enable auto dump Hprof , sava path:"+flags);
            SendMsgForSystemHprof(flags);
            return true;
        }
        return false;
    }

    private void processTraceViewMsg(Message msg){
        if (msg.arg1 == 1){
            openDebugTraceView(msg);
        }else if(msg.arg1 == 0){
             closeDebugTraceView(msg);
        }else if(msg.arg1 == 2){
             dumpCurProcessHprof(msg);
        }
    }

    private void SendMsgForSystemHprof(String path){
        long when = 0;
        Message msg = mH.obtainMessage(START_TRACEVIEW_MSG);
        msg.arg1 = 2;
        msg.obj   =  path;
        long now = SystemClock.uptimeMillis();
        when = now + 10; // delay 10 ms
        mH.sendMessageAtTime(msg, when);
    }

    private void dumpCurProcessHprof(Message msg){
        try{
            String  array = (String)(msg.obj);
            String [] flagArray  =  array.split(",");
            String path = flagArray[1];
            android.os.Debug.dumpHprofData(path);
            Slog.w(TAG, "end dumpHrpofData!!! save path = " + path);
        }catch(IOException e) {
            Slog.w(TAG, "dumpHrpofData  error!"+e);
        }
    }

    private void openDebugTraceView(Message msg){
        if(TRACEVIEW_CUR_FILE != null){
            Slog.w(TAG, "has catching traceView, pls close debug traceView first!");
            return;
        }
        String  array = (String)(msg.obj);
        String [] flagArray  =  array.split(",");
        String path = flagArray[1];
        TRACEVIEW_CUR_FILE = path;
        Slog.w(TAG, "starting catch traceView !!! tr file:"+TRACEVIEW_CUR_FILE);
        Debug.startMethodTracingSamplingEx(TRACEVIEW_CUR_FILE, 32 * 1024 * 1024, 250);
    }

    private void closeDebugTraceView(Message msg){
        if(TRACEVIEW_CUR_FILE == null){
            Slog.w(TAG, "do not catch traceView, pls open debug traceView first !!!");
            return;
        }

        Debug.stopMethodTracing();
        TRACEVIEW_CUR_FILE = null;
        Slog.w(TAG, "stop catch traceView !! saving :"+TRACEVIEW_CUR_FILE);
    }

    private void autoCatchTraceView(int timeout, String path){
        long when = 0;
        Message msg = mH.obtainMessage(START_TRACEVIEW_MSG);
        msg.arg1 = 1;
        msg.obj   = path;
        long now = SystemClock.uptimeMillis();
        when = now + 10; // delay 100 ms
        mH.sendMessageAtTime(msg, when);
        Message msg1 = mH.obtainMessage(START_TRACEVIEW_MSG);
        msg1.arg1 = 0;
        msg1.obj   = path;
        when +=  timeout;
         mH.sendMessageAtTime(msg1, when);
    }
    //[+LEUI] [TASK][RUBY-20880] [xulei] end 
}
