...
public abstract class FragmentManager {
    ...
    //抽象方法,具体实现在FragmentManagerImpl中
...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflater.Factory2 {
    ...
    //开始一个Transaction, new BackStackRecord
    @Override
        ...
        if (f.mState < newState) {
            ...
            switch (f.mState) {
                ...
                case Fragment.CREATED:
                    //在每个case检测
                    //如果Activity的状态大于Fragment的状态,则继续初始化fragment
                    if (newState > Fragment.CREATED) {
                        ...
                        if (!f.mFromLayout) {
                            ...
                            //createView 在 activityCreated之前
//                            here
                            f.mView = f.performCreateView(f.getLayoutInflater(
    ...
    public void enqueueAction(Runnable action, boolean allowStateLoss) {
        ...
        synchronized (this) {
            ...
            //将事务添加到Runnable的ArrayList中
            mPendingActions.add(action);
            if (mPendingActions.size() == 1) {
                mHost.getHandler().removeCallbacks(mExecCommit);
                //使用主线程的Handler执行mExecCommit:Runnable
                /*Runnable mExecCommit = new Runnable() {
                    @Override
                    public void run() {
                        execPendingActions();
    ...
    public boolean execPendingActions() {
        ...
        while (true) {
            ...
            for (int i=0; i<numActions; i++) {
                //调用事务Transaction: BackStackRecord的run方法
                mTmpActions[i].run();
                mTmpActions[i] = null;
            }
