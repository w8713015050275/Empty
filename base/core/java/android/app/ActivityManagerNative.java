...
/** {@hide} */
//类似AIDL的Stub, 在服务端被调用 onTransact()
    //有服务端实例继承这个接口
public abstract class ActivityManagerNative extends Binder implements IActivityManager
{
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags)
        ...
        //[add start 20160314][yuanguohong][LEUI-13164]
        case FORCE_STOP_PACKAGE_TRANSACTION_FOR_EUI: {
            data.enforceInterface(IActivityManager.descriptor);
            String packageName = data.readString();
            int userId = data.readInt();
            forceStopPackageForEUI(packageName, userId);
            reply.writeNoException();
            return true;
        }
        //[add end 20160314]
        ...
        //[add start 20151106][yuanguohong][MOBILEP-12012]
        case REMOVE_TASK_THROUGH_TRANSACTION:
        {
            data.enforceInterface(IActivityManager.descriptor);
            int taskId = data.readInt();
            int fl = data.readInt();
            boolean isThrough = data.readInt() != 0;
            boolean result = removeTaskThrough(taskId, fl, isThrough);
            reply.writeNoException();
            reply.writeInt(result ? 1 : 0);
            return true;
        }
        ...
        case CHECK_CLONE_BY_PID_TRANSACTION:
        {
            data.enforceInterface(IActivityManager.descriptor);
            int pid = data.readInt();
            boolean result = isCloneProcessByPid(pid);
            reply.writeNoException();
            reply.writeInt(result ? 1 : 0);
            return true;
        }
        ...
        /// M: ALPS00431397[MOBILEP-7983][RUBY-7961]. When running window animation, halt the activity
        ...
        // [+LEUI] [REQ] [LEUI-13225] [yuanguohong] added:feature for limit background apps.
        case CHECK_ACQUIRE_RESOURCE_TRANSACTION:
        {
            data.enforceInterface(IActivityManager.descriptor);
            String tag = data.readString();
            String packageName = data.readString();
            ...
            int pid = data.readInt();
            int type = data.readInt();
            int result = isAllowAcquireResource(tag, packageName, uid, pid, type);
            reply.writeNoException();
            reply.writeInt(result );
            return true;
        }
    ...
    private static final Singleton<IActivityManager> gDefault = new Singleton<IActivityManager>() {
        protected IActivityManager create() {
            //从ServiceManager中去get到IBinder: ActivityManagerService
            IBinder b = ServiceManager.getService("activity");
            if (false) {
                Log.v("ActivityManager", "default service binder = " + b);
            }
            //client端获得代理类, 调用相应的方法
            //client端调用,得到ActivityManagerProxy
            //调用ActivityManagerProxy的方法 --> 调用mRemote, mRemote为远程IBinder对象
...
class ActivityManagerProxy implements IActivityManager
{
    ...
    public void setLockScreenShown(boolean shown) throws RemoteException
    {
        ...
        mRemote.transact(SET_LOCK_SCREEN_SHOWN_TRANSACTION, data, reply, IBinder.FLAG_ONEWAY);
        reply.readException();
        data.recycle();
        reply.recycle();
    }
    ...
    //[add start 20160314][yuanguohong][LEUI-13164]
    public void forceStopPackageForEUI(String packageName, int userId) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeString(packageName);
        data.writeInt(userId);
        mRemote.transact(FORCE_STOP_PACKAGE_TRANSACTION_FOR_EUI, data, reply, 0);
        reply.readException();
        data.recycle();
        reply.recycle();
    }
    ...
    //[add start 20151106][yuanguohong][MOBILEP-12012]
    public boolean removeTaskThrough(int taskId, int flags, boolean isThrough) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeInt(taskId);
        data.writeInt(flags);
        data.writeInt(isThrough ? 1 : 0);
        mRemote.transact(REMOVE_TASK_THROUGH_TRANSACTION, data, reply, 0);
        reply.readException();
        boolean result = reply.readInt() != 0;
        reply.recycle();
        data.recycle();
	...
	//[+LEUI] [REQ][LEUI-8957] [xulei] add 	
    //for support double app 
    public boolean isCloneProcessByPid(int pid)throws RemoteException{
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeInt(pid);
        mRemote.transact(CHECK_CLONE_BY_PID_TRANSACTION, data, reply, 0);
        reply.readException();
        boolean result = reply.readInt() != 0;
        reply.recycle();
        data.recycle();
    ...
    // [+LEUI] [REQ] [LEUI-13225] [yuanguohong] added:feature for limit background apps.
    public int isAllowAcquireResource(String tag, String packageName, int uid, int pid, int resourceType) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeString(tag);
        ...
        data.writeInt(uid);
        data.writeInt(pid);
        data.writeInt(resourceType);
        mRemote.transact(CHECK_ACQUIRE_RESOURCE_TRANSACTION, data, reply, 0);
        reply.readException();
        int result = reply.readInt();
        reply.recycle();
        data.recycle();
