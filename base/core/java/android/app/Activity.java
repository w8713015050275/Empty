...
//[+LEUI][LEUI-9982][guojian@letv.com] 
import com.letv.leui.util.statusbar.LeImmersiveStatusBarManager;
import com.letv.leui.util.statusbar.StatusbarColorFromTitleHelper;
import com.letv.leui.util.statusbar.StatusbarColorInfo;
...
public class Activity extends ContextThemeWrapper
    ...
    public void onBackPressed() {
        ...
        //[+LEUI] [BUG][RUBY-5539] [wubo] added
        if (((FragmentManagerImpl)mFragments.getFragmentManager()).mStateSaved) {
            return;
        }
        //[-LEUI]
    ...
    //[+LEUI][TASK][LEUI-25384][Improve special app drag cpu boost level][nietong]
    private long mBeginTime = System.currentTimeMillis();
    private long mEndTime = 0;
    private static final int DRAG_BOOST_INTERVAL_TIME = 180;
    private static final int DRAG_BOOST_DUARION_TIME = 200;

    //实现的Window.Callback的dispatchTouchEvent方法
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if(mDragBoostPossible == -1) {
            ...
            String[] activityList = getResources().getStringArray(
                com.android.internal.R.array.le_boost_highlevel_scroller_packages_list);
       ...
       if (mDragBoostPossible == 1 && ((mEndTime - mBeginTime) >= DRAG_BOOST_INTERVAL_TIME ) && override != true) {

            if (mPerf == null){
                mPerf = new BoostFramework();
            }
            if(mPerfLockDuration == -1){
                mPerfLockDuration = DRAG_BOOST_DUARION_TIME;
                mAsParamVal = getResources().getIntArray(
                    com.android.internal.R.array.highlevel_scrollerboost_param_value);
            }
        ...
        if (getWindow().superDispatchTouchEvent(ev)) {
            //执行PhoneWindow的superDispatchTouchEvent;返回true表示有view处理这个事件了
            return true;
        }
        //如果ViewGroup返回false; 没有任何View处理Touch事件
