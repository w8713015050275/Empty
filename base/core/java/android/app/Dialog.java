...
public class Dialog implements DialogInterface, Window.Callback,
    ...
    Dialog(@NonNull Context context, @StyleRes int themeResId, boolean createContextThemeWrapper) {
        ...
        //dialog的窗口的初始化
        //先get到Window的服务,实际为WindowManagerImpl对象
        //WindowManagerImpl实现WindowManager接口,WIndowManager继承ViewManager
        //这里的mWindowManager,只有Display参数构造
//        return new WindowManagerImpl(ctx.getDisplay());
        ...
        mWindow = w;
        //把一系列回调方法设置给PhoneWindow对象
        //dialog要实现Window.Callback接口:事件分发啥的调用
        //是谁回调的呢???
        w.setCallback(this);
        w.setOnWindowDismissedCallback(this);
        //mWindowManager为进程唯一的WindowManagerImpl对象,继承了WindowManager;
        //WindowManagerImpl真正的实现为WindowManagerGlobal,也是进程唯一的,WMG中获得WindowManagerService
        //Window和WindowManager建立联系;实际和WindowManagerImpl建立联系
        //重新new了一个WindowManagerImpl
        w.setWindowManager(mWindowManager, null, null);
        //窗口居中
        w.setGravity(Gravity.CENTER);

        //回调onDismiss\onCancle等
