...
import com.letv.leui.util.appsnapshot.AppSnapshotImpl;
import com.letv.eui.appsnapshot.IAppSnapshotWatcher;
...
public final class ViewRootImpl implements ViewParent,
    ...
    private boolean LOCAL_LOGV = false;
    /** @noinspection PointlessBooleanExpression*/
    private boolean DEBUG_DRAW = false || LOCAL_LOGV;
    private boolean DEBUG_LAYOUT = false || LOCAL_LOGV;
    private boolean DEBUG_DIALOG = false || LOCAL_LOGV;
    private boolean DEBUG_INPUT_RESIZE = false || LOCAL_LOGV;
    private boolean DEBUG_ORIENTATION = false || LOCAL_LOGV;
    private boolean DEBUG_TRACKBALL = false || LOCAL_LOGV;
    private boolean DEBUG_IMF = false || LOCAL_LOGV;
    private boolean DEBUG_CONFIGURATION = false || LOCAL_LOGV;
    private boolean DEBUG_FPS = false;
    private boolean DEBUG_INPUT_STAGES = false || LOCAL_LOGV;

    private boolean DEBUG_INPUT = false;
    private boolean DEBUG_KEY = false || DEBUG_INPUT;
    private boolean DEBUG_MOTION = false || DEBUG_INPUT;
    // For debugging IME caused key dispatch timeout ANR.
    private boolean DEBUG_IME_ANR = false;
    private boolean DEBUG_LIFECYCLE = false || LOCAL_LOGV;
    private boolean DEBUG_REQUESTLAYOUT = false;
    private boolean DEBUG_INVALIDATE = false;
    private boolean DEBUG_SCHEDULETRAVERSALS = false;
    private boolean DEBUG_TOUCHMODE = false;
    @SuppressWarnings("staticvariablename")
    private boolean DEBUG_MEASURE_LAYOUT = false || LOCAL_LOGV;

    /** Add debug flags to partial enable logs. @{ */
    ...
    //[+LEUI][REQ][LAFITEN-42][xueqiushi] Add HW Acceleration debug properties.
    private static final boolean DEBUG_HW_STATUS = SystemProperties.getBoolean(
            "debug.viewroot.HW_status", false);
    ...
    /// instance id used to identify each ViewRootImpl instance.
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        //[RUBY-20880][xulei]set context
        mChoreographer.setContext(mContext.getApplicationContext());
        //end
    ...
    private void enableHardwareAcceleration(WindowManager.LayoutParams attrs) {
        ...
        //[+LEUI][REQ][LAFITEN-42][xueqiushi] Disable HW Acceleration by adb command
        if (DEBUG_DISABLEHW) {
        ...
        //[+LEUI][REQ][LAFITEN-42][xueqiushi] APP doesn't declare hardware accelerated
        if (!hardwareAccelerated && DEBUG_HW_STATUS) {
        ...
        if (hardwareAccelerated) {
            if (!HardwareRenderer.isAvailable()) {
                //[+LEUI][REQ][LAFITEN-42][xueqiushi] System doesn't support hardware acceleration
                if (DEBUG_HW_STATUS)
            ...
            if (fakeHwAccelerated) {
                ...
                if (mAttachInfo.mHardwareRenderer != null) {
                    ...
                    mAttachInfo.mHardwareAccelerated =
                            mAttachInfo.mHardwareAccelerationRequested = true;
                    //[+LEUI][REQ][LAFITEN-42][xueqiushi] Hardware acceleration is enabled
                    if (DEBUG_HW_STATUS)
                        Log.d(TAG, "hardware acceleration is enabled, this = " + this);
                    //[-LEUI]
                }
            }
            //[+LEUI][REQ][LAFITEN-42][xueqiushi] Hardware acceleration is disabled due to following reasons.
            if (mAttachInfo.mHardwareRenderer == null && DEBUG_HW_STATUS) {
        ...
        if (!mHandlingLayoutInLayoutRequest) {
            ...
            if (DEBUG_LAYOUT) {
                Log.d(TAG, "requestLayout: mView = " + mView + ", this = " + this ,
                        DEBUG_REQUESTLAYOUT ? new Throwable("requestLayout") : null);
            }
    ...
    public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
        ...
        if (DEBUG_DRAW)
            Log.v(TAG, "Invalidate child: " + dirty + ",this = " + this,
                    DEBUG_INVALIDATE ? new Throwable("invalidateChildInParent") : null);
    ...
    void setWindowStopped(boolean stopped) {
        if (DEBUG_LAYOUT) {
            Log.v(TAG, "setStopped: stopped = " + stopped + ", this = " + this);
        }
        if (mStopped != stopped) {
            mStopped = stopped;
            if (!mStopped) {
        ...
        if (!mTraversalScheduled) {
            ...
            if (DEBUG_LIFECYCLE) {
                Log.v(TAG, "scheduleTraversals: mTraversalBarrier = " + mTraversalBarrier
                        + ",this = " + this, DEBUG_SCHEDULETRAVERSALS ? new Throwable("scheduleTraversals") : null);
            }
    ...
    private boolean measureHierarchy(final View host, final WindowManager.LayoutParams lp,
        ...
        if (lp.width == ViewGroup.LayoutParams.WRAP_CONTENT) {
            ...
            if (DEBUG_DIALOG) Log.v(TAG, "Window " + mView + ": baseSize=" + baseSize + ", this = " + this);
            if (baseSize != 0 && desiredWindowWidth > baseSize) {
                childWidthMeasureSpec = getRootMeasureSpec(baseSize, lp.width);
                childHeightMeasureSpec = getRootMeasureSpec(desiredWindowHeight, lp.height);
                performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
                if (DEBUG_DIALOG) Log.v(TAG, "Window " + mView + ": measured ("
                        + host.getMeasuredWidth() + "," + host.getMeasuredHeight() + ") " + this);
    ...
    private void performTraversals() {
        ...
        if (mFirst) {
            ...
            if (desiredWindowWidth != mWidth || desiredWindowHeight != mHeight) {
                if (DEBUG_ORIENTATION || DEBUG_LAYOUT) Log.v(TAG,
                        "View " + host + " resized to: " + frame + ",this = " + this);
                mFullRedrawNeeded = true;
                mLayoutRequested = true;
                windowSizeMayChange = true;
            }
        ...
        if (viewVisibilityChanged) {
            ...
            com.letv.leui.widget.text.clipboard.LeClipboardManager manager =
                    (com.letv.leui.widget.text.clipboard.LeClipboardManager) mView.getContext().getSystemService(
                    Context.LE_CLIPBOARD_SERVICE);
            if (manager.isLeClipboardShowing(mContext) && manager.getRootView() == mView) {
                manager.onWindowVisibility(mContext, viewVisibility);
            }
            //[-LEUI]
            mAttachInfo.mWindowVisibility = viewVisibility;
        ...
        if (layoutRequested) {
            ...
            if (mFirst) {
                ...
                if (!mPendingContentInsets.equals(mAttachInfo.mContentInsets)) {
                    if (DEBUG_LAYOUT) {
                        Log.v(TAG, "Content insets changing to " + mPendingContentInsets
                                + " from: " + mAttachInfo.mContentInsets + ", this = " + this);
                    }
                    insetsChanged = true;
                }
                ...
                if (!mPendingVisibleInsets.equals(mAttachInfo.mVisibleInsets)) {
                    if (DEBUG_LAYOUT) Log.v(TAG, "Visible insets changing to: " + mPendingVisibleInsets
                            + " from: " + mAttachInfo.mVisibleInsets
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
            ...
            if (!mStopped || mReportNextDraw) {
                ...
                if (focusChangedDueToTouchMode || mWidth != host.getMeasuredWidth()
                    ...
                    //根据DecorView的宽高,窗口lp的宽高 生成MeasureSpec
                    ...
                    if (DEBUG_LAYOUT) {
                     ...
                     // Ask host how big it wants to be
                    //测量 DecorView FrameLayout ViewGroup
                    performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
        ...
        // [+LEUI-9331]
        boolean blurVisibilityChanged = viewVisibilityChanged || mFirst;
        if (!mWindowsAnimating || blurVisibilityChanged) {
            BlurParams curBp = null;
            boolean hasBlur = false;
            boolean needSetBlur = false;
            View curBlurView = mAttachInfo.mViewRequestingWindowBlur;
            if (curBlurView != null) {
                curBp = curBlurView.getBlurParams();

                if (curBp != null && curBlurView.getVisibility() == View.VISIBLE) {
                    curBlurView.getRectInWindow(curBp.area);

                    float blurAlpha = curBlurView.getBlurAlpha();
                    if (blurAlpha < 1.0 - BlurParams.BLUR_FLT_EPSILON) {
                        curBp = BlurParams.lerp(BlurParams.EMPTY, curBp, blurAlpha);
                    }

                    if (curBp.mode == BlurParams.BLUR_MODE_WINDOW && !curBp.isEmpty()) {
                        hasBlur = true;
                        if (!curBp.equals(mPreBlurParams) || blurVisibilityChanged) {
                            // add or update blur
                            needSetBlur = true;
                        }
                    }
            ...
            if (!hasBlur && mPreBlurParams.mode == BlurParams.BLUR_MODE_WINDOW) {
                // remove blur
                curBp = null;
                needSetBlur = true;
            }
            ...
            if (needSetBlur) {
                if (curBp != null) Log.d(TAG, "setBlur rc(" + curBp.area.toShortString() + ") r(" + curBp.radius + ") " + mWindowAttributes.getTitle());
                else Log.d(TAG, "setBlur null " + mWindowAttributes.getTitle());

                if (curBp != null) mPreBlurParams.set(curBp);
                else mPreBlurParams.mode = BlurParams.BLUR_MODE_NONE;

                try {
                    mWindowSession.setBlur(mWindow, curBp);
                } catch(RemoteException e) {
                    Log.d(TAG,"setBlur failed.");
                }
            }
        }
        // [-LEUI-9331]
    ...
    private void trackFPS() {
        ...
        if (mFpsStartTime < 0) {
            ...
            Log.v(TAG, "Frame time:\t" + frameTime + " " + this);
            mFpsPrevTime = nowTime;
            if (totalTime > 1000) {
                float fps = (float) mFpsNumFrames * 1000 / totalTime;
                Log.v(TAG, "FPS:\t" + fps + " " + this);
    ...
    void dispatchDetachedFromWindow() {
        ...
        com.letv.leui.widget.text.clipboard.LeClipboardManager manager =
                ...
                Context.LE_CLIPBOARD_SERVICE);
        if (manager.isLeClipboardShowing(mContext) && manager.getRootView() == mView) {
            manager.hideLeClipboard(mContext);
        }
        //[-LEUI]
    ...
    private final static int MSG_EUI_INDEX = 100;
    ...
    //[+LEUI-10031]
    private final static int MSG_START_APP_SNAPSHOT = MSG_EUI_INDEX + 0;
    private final static int MSG_STOP_APP_SNAPSHOT = MSG_EUI_INDEX +1;
    private final static int MSG_FAILED_APP_SNAPSHOT = MSG_EUI_INDEX +2;
    //[-LEUI-10031]

    private final static int MSG_DUMP_VIEW = MSG_EUI_INDEX +3;
                ...
                if (mAdded) {
                    ...
                    if (mView != null) {
                        ...
                        com.letv.leui.widget.text.clipboard.LeClipboardManager manager =
                                (com.letv.leui.widget.text.clipboard.LeClipboardManager) mView.getContext().getSystemService(
                                Context.LE_CLIPBOARD_SERVICE);
                        if (manager.isLeClipboardShowing(mContext)) {
//                                manager.setServedView(null);
//                            manager.hideLeClipboard(mContext);
                            manager.onWindowFocusChanged(mContext, hasWindowFocus? 1:0);
                        }
                        if (hasWindowFocus) {
                            manager.setRootView(mView);
                        }
                        //[-LEUI]
                        mView.dispatchWindowFocusChanged(hasWindowFocus);
                        mAttachInfo.mTreeObserver.dispatchOnWindowFocusChange(hasWindowFocus);
                        //[+LEUI-21622]ONLY update immersive mode when window gain focus.
                        if(hasWindowFocus){
                            if(mView instanceof RootViewSurfaceTaker){
                                Log.i(TAG,"updateImmersive mode: gain focus");
                                ((RootViewSurfaceTaker)mView).updateImmersiveMode();
                            }
                        }
                        //[-LEUI-21622]
                    }
            ...
            // [LEUI-10031]
            case MSG_START_APP_SNAPSHOT : {
                IAppSnapshotWatcher watcher = (IAppSnapshotWatcher)msg.obj;
                startAppSnapshot(watcher);
            } break;
            case MSG_STOP_APP_SNAPSHOT: {
                stopAppSnapshot();
            } break;
            case MSG_FAILED_APP_SNAPSHOT: {
                IAppSnapshotWatcher watcher = (IAppSnapshotWatcher)msg.obj;
                int resultCode = msg.arg1;
                failedAppSnapshot(watcher, resultCode);
            }break;
            //[-LEUI-10031]
            // [+LEUI] [REQ][LEUI-26761] [yueshuai] add
            case MSG_DUMP_VIEW:{
                dumpView((SomeArgs) msg.obj);
            }break;
            // [+LEUI] [REQ][LEUI-26761]
            case MSG_DISPATCH_WINDOW_SHOWN: {
    ...
    final class SyntheticTrackballHandler {
        private final TrackballAxis mX = new TrackballAxis(ViewRootImpl.this);
        private final TrackballAxis mY = new TrackballAxis(ViewRootImpl.this);
    ...
    static final class TrackballAxis {
        ...
        TrackballAxis(ViewRootImpl viewAncestor) {
        ...
        float collect(float off, long time, String axis) {
            ...
            if (off > 0) {
                ...
                if (dir < 0) {
                    ...
                    if (viewRoot != null && viewRoot.DEBUG_TRACKBALL)
    ...
    public void handleDispatchWindowAnimationStarted(int remainingFrameCount) {
        if (DEBUG_DRAW) {
            Log.v(TAG, "handleDispatchWindowAnimationStarted: mDrawDuringWindowsAnimating = "
                    + mDrawDuringWindowsAnimating + ", mWindowsAnimating = "
                    + mWindowsAnimating + ", remainingFrameCount = " + remainingFrameCount + ", this = " + this);
        }
        if (!mDrawDuringWindowsAnimating && remainingFrameCount != -1) {
            mRemainingFrameCount = remainingFrameCount;
            mWindowsAnimating = true;
            if (DEBUG_DRAW) {
                Log.v(TAG, "Set mWindowsAnimating to true, this = " + this);
            }
        }
    }
    ...
    public void handleDispatchWindowAnimationStopped() {
        if (DEBUG_DRAW) {
            Log.v(TAG, "handleDispatchWindowAnimationStopped: mDirty = " + mDirty + ", mWindowsAnimating = "
    ...
    // [+LEUI] [REQ][LEUI-26761] [yueshuai] add
    private void dumpView(SomeArgs msg){
        final StrictMode.ThreadPolicy oldPolicy = StrictMode.allowThreadDiskWrites();
        ParcelFileDescriptor fd = null;
        try {
            String prefix = (String)msg.arg1;
            fd = (ParcelFileDescriptor)msg.arg2;
            String[] args = (String[])msg.arg3;
            PrintWriter pw = new FastPrintWriter(new FileOutputStream(fd.getFileDescriptor()));
            dump(prefix, fd.getFileDescriptor(), pw, args);
            pw.flush();
            msg.recycle();
        }finally {
            IoUtils.closeQuietly(fd.getFileDescriptor());
    ...
    // [+LEUI] [REQ][LEUI-26761] [yueshuai] add
    public void dispatchDumpViewHierarchy(String prefix, ParcelFileDescriptor fd, String[] args) {
        SomeArgs sArgs = SomeArgs.obtain();
        try{
            sArgs.arg1 = prefix;
            sArgs.arg2 = fd;
            sArgs.arg3 = args;
            Message msg = mHandler.obtainMessage(MSG_DUMP_VIEW);
            msg.obj = sArgs;
            msg.setAsynchronous(true);
            mHandler.sendMessage(msg);
        }catch(Exception e){
            Slog.w(TAG, "dumpViewHierarchy failed", e);
        }
    }
    // [+LEUI] [REQ][LEUI-26761]
    ...
    private void finishInputEvent(QueuedInputEvent q) {
        ...
        if (inputElapseTime >= INPUT_TIMEOUT) {
            ...
            Log.v(TAG, "[ANR Warning]Input routeing takes more than 4000ms since "
                    + simpleDateFormat.format(enqueueTime) + ", this = " + this);
        }
        if (q.mReceiver != null) {
            handled = (q.mFlags & QueuedInputEvent.FLAG_FINISHED_HANDLED) != 0;
        ...
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Log.v(TAG, "finishInputEvent: handled = " + handled +  ",event = " + q.mEvent
                    + ",stage=" + stage + ",inputElapseTime="+inputElapseTime+" , this = " + this);
        }else{
            if(q.mEvent instanceof KeyEvent){
                final KeyEvent keye = (KeyEvent)q.mEvent;
                if(keye.getAction() == KeyEvent.ACTION_DOWN ) {
                    Log.i(TAG, "finishKeyEvent: handled = " + handled + " keycode = " + keye.getKeyCode()
                            +" name=" + keye.keyCodeToString(keye.getKeyCode()) +" stage=" + stage
                            + ",inputElapseTime="+inputElapseTime+" eventTime = " + keye.getEventTime()
                            + " downTime = " +keye.getDownTime() +  " title= "+ this.mWindowAttributes.getTitle());
                }
            }else if(q.mEvent instanceof MotionEvent){
                final MotionEvent me = (MotionEvent)q.mEvent;
                if(me.getAction() == MotionEvent.ACTION_DOWN ) {
                    Log.i(TAG, "finishMotionEvent: handled = " + handled + " stage=" + stage
                            + ",inputElapseTime="+inputElapseTime+" eventTime = " + me.getEventTime()
                            + " downTime = " +me.getDownTime()+ " title= "+ this.mWindowAttributes.getTitle());
                }
    ...
    static class W extends IWindow.Stub {
        ...
        public void dispatchAppVisibility(boolean visible) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                if (viewAncestor.DEBUG_LIFECYCLE) {
        ...
        public void dispatchDragEvent(DragEvent event) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                if (viewAncestor.LOCAL_LOGV || viewAncestor.DEBUG_INPUT) {
        ...
        public void onAnimationStarted(int remainingFrameCount) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                if (viewAncestor.DEBUG_DRAW) {
                    Log.v(TAG, "onAnimationStarted(" + remainingFrameCount + "): viewAncestor" + viewAncestor);
                }
        ...
        public void onAnimationStopped() {
            ...
            if (viewAncestor != null) {
                if (viewAncestor.DEBUG_DRAW) {
                    Log.v(TAG, "onAnimationStopped: viewAncestor" + viewAncestor);
                }
        ...
        // API used to switch on/off debug flags.
        @Override
        public void enableLog(boolean enable, String flags) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.enableLog(enable, flags);
            }
        }

        //[+LEUI-10031]
        @Override
        public void startAppSnapshot(IAppSnapshotWatcher watcher){
            Log.i(TAG,"startAppSnapShot watcher="+watcher);
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchStartAppSnapshot(watcher);
            }
        }

        @Override
        public void stopAppSnapshot(){
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchStopAppSnapshot();
            }
        }
        //[-LEUI-10031]
        ...
        // [+LEUI] [REQ][LEUI-26761] [yueshuai] add
        @Override
        public void dumpViewHierarchy(ParcelFileDescriptor fd, String prefix, String[] args) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                try {
                    ParcelFileDescriptor client = ParcelFileDescriptor.dup(fd.getFileDescriptor());
                    viewAncestor.dispatchDumpViewHierarchy(prefix, client, args);
                }catch (Exception e){
                    Slog.e(TAG, "fd dup failed");
                }finally {
                    try {
                        fd.close();
                    } catch (IOException e) {
     ...
     */
    void enableLog(boolean enable, String flags) {
        Log.v(TAG, "enableLog: enable = " + enable + ", flag = " + flags);
        if (flags == null || flags.length() == 0)
            return;
        String[] flagArray = flags.split(",");
        if (flagArray[0].equals("default")) {
            LOCAL_LOGV = enable;
            DEBUG_DRAW = enable;
            DEBUG_LAYOUT = enable;
            DEBUG_DIALOG = enable;
        ...
        for (int i = 0; i < flagArray.length; i++) {
            try {
                count = Integer.parseInt(flagArray[i]);
            ...
            switch (count) {
                ...
                case 16:
                    ...
                    Looper.getMainLooper().setMessageLogging(print);
                    break;
                case 17:
                    // debug Choreographer
                    mChoreographer.enableDebug(enable);
                    break;
            }
        }
    }
    ...
    /**
     * Get ViewRoot logs property and partial enable logs if debug flags is
     ...
     */
    private void checkViewRootImplLogProperty() {
        ...
        if (propString != null && propString.length() > 0) {
            ...
            try {
                ...
                Log.w(TAG, "Invalid format of propery string: " + propString + " " + this);
            }

            Log.d(TAG, "checkViewRootImplLogProperty: propString = " + propString
                    + ",logFilter = #" + Integer.toHexString(logFilter) + " " + this);
    ...
    //[+LEUI-10031]
    public void dispatchStartAppSnapshot(IAppSnapshotWatcher watcher){
        if(mCurrentWatcher != null){
            Message msg = mHandler.obtainMessage(MSG_FAILED_APP_SNAPSHOT,
                    AppSnapshotImpl.SNAPSHOT_OCCUPIED_FAILED, 0, watcher);
            mHandler.sendMessage(msg);
        }else{
            Message msg = mHandler.obtainMessage(MSG_START_APP_SNAPSHOT, watcher);
            mHandler.sendMessage(msg);
        }
    }

    public void dispatchStopAppSnapshot(){
        Message msg = mHandler.obtainMessage(MSG_STOP_APP_SNAPSHOT);
        mHandler.sendMessage(msg);
    }
    ...
    public void startAppSnapshot(IAppSnapshotWatcher watcher){
        ...
        if(mCurrentWatcher == null){
            try{
                ...
                mCurrentWatcher.asBinder().linkToDeath(mCurrentWatcherDr, 0);
            }catch(RemoteException e){
            }
            startScreenScrollShoot(null);
        }else{
            Log.e(TAG,"Can NOT startAppSnapshot.");
        }
    }

    public void stopAppSnapshot(){
        forceStopScrollShoot();
    }

    public void failedAppSnapshot(IAppSnapshotWatcher watcher, int resultCode){
        if(watcher != null){
            try{
                watcher.onSnapshotFailed(resultCode);
            }catch(RemoteException e){
                Log.e(TAG,"Failed to call failedAppSnapshot");
            }
        }
    }
    //[-LEUI-10031]
    ...
    private static final int INPUT_TIMEOUT = 4000;
    /// @}

    IAppSnapshotWatcher mCurrentWatcher;
    IBinder.DeathRecipient mCurrentWatcherDr = new IBinder.DeathRecipient() {
        @Override
        public void binderDied() {
            synchronized (this) {
                Log.i(TAG,"mCurrentWatcher="+mCurrentWatcher+" binderDied !!!");
                if(mCurrentWatcher != null){
                    IBinder binder = mCurrentWatcher.asBinder();
                    if(binder != null){
                        binder.unlinkToDeath(this, 0);
                        mCurrentWatcher = null;
                    }
                }
            }
        }
    };
//[+LEUI][liukai6@letv.com] for screen scroll shoot for thidrpart app.
    private AppSnapshotImpl mScreenScrollShootHelper;

    public void startScreenScrollShoot(Bitmap bitmap){
        Log.i(TAG,"startScreenScrollShoot");
        if(mWindowAttributes != null){
            Log.i(TAG," title="+mWindowAttributes.getTitle());
        }
        if(mScreenScrollShootHelper == null){
        if(mView == null) return;
            mScreenScrollShootHelper = new AppSnapshotImpl(mContext,mView);

        mScreenScrollShootHelper.setScreenShootAdapter(new AppSnapshotImpl.ScreenShootAdapter(){
        @Override
        public Bitmap getScreenShootPic(){
            Bitmap bitmap = null;
            if(mSurface != null && mSurface.isValid()){
             bitmap = mSurface.captureSurface();
             if(bitmap != null){
                Log.i(TAG,"bitmap width="+bitmap.getWidth()+" height="+bitmap.getHeight());
             }else{
                Log.e(TAG,"getScreenShotPic can NOT capture suface");
             }
            }
           return bitmap;
        }
            ...
            mScreenScrollShootHelper.setScrollShootListener(new AppSnapshotImpl.ScrollShootListener() {
                @Override
                public void onStart() {
                    //TODO callback
                    if(mCurrentWatcher != null){
                        try{
                        mCurrentWatcher.onSnapshotStart();
                        }catch(RemoteException e){
                        }
                    }
                }
                ...
                public void onFinished(Bitmap bitmap) {
                    //TODO callback
                    Log.i(TAG,"onFinished bitmap="+bitmap+" mCurrentWatcher="+mCurrentWatcher);
                    if(mCurrentWatcher != null){
                        try{
                            mCurrentWatcher.onSnapshotFinished(bitmap);
                            mCurrentWatcher = null;
                        }catch(RemoteException e){
                        }
                    }
                ...
                public void onFailed(int resultCode){
                    Log.i(TAG,"onFailed resultCode="+resultCode);
                }
            });
        }
        Log.i(TAG," do ScreenScrollShootHelper  scrollShootStart");
        mScreenScrollShootHelper.scrollShootStart(bitmap);
    }

    //Useless
    class SnapshotWatcher {
        IAppSnapshotWatcher watcher;
        IBinder.DeathRecipient deathRecipient;
        SnapshotWatcher(IAppSnapshotWatcher w, IBinder.DeathRecipient d) {
            watcher = w;
            deathRecipient = d;
        }
    }

    public void forceStopScrollShoot(){
        if(mScreenScrollShootHelper == null){
            //TODO shoot not start
            return;
        }
        mScreenScrollShootHelper.forceStop();
    }
    //[-LEUI]
}
