...
import android.graphics.BlurPresets;
// [-LEUI-9331]
...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    */
    ...
    /*< LAFITE-769 yangyanhe 2016-01-21 begin*/
    /**
     * @hide
     * Flag for {@link #setSystemUiVisibility(int)}:View would do nothing when user touch,move
     * the status bar.
     */
    public static final int SYSTEM_UI_FLAG_IMMERSIVE_GESTURE_ISOLATED = 0x00004000;
    /* LAFITE-769 yangyanhe 2016-01-21 end >*/
    ...
    // [+LEUI-9331]
    /**
    * Blur effect parameters of this view.
    */
    private BlurParams mBlurParams = new BlurParams();
    ...
    public boolean performClick() {
        //[+LEUI]add for LEUI-21850 by weijiabai begin
        sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_CLICKED);
        //[-LEUI]add for LEUI-21850 by weijiabai end
        ...
        //[+LEUI]modify for LEUI-21850 by weijiabai begin
        //sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_CLICKED);
        //[-LEUI]modify for LEUI-21850 by weijiabai end
        return result;
    }
    ...
    */
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (onFilterTouchEventForSecurity(event)) {
            ...
            if (li != null && li.mOnTouchListener != null
                    ...
                    //onTouchListener.onTouch返回true
                    && li.mOnTouchListener.onTouch(this, event)) {
                //[+LEUI]add for LEUI-21850 by weijiabai begin
                if (AccessibilityManager.getInstance(mContext).isNiTrackerEnabled()) {
                     sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_ONTOUCH);
                }
                 //[-LEUI]add for LEUI-21850 by weijiabai end
            ...
            //普通View的onTouchEvent()
            //onTouchListener.onTouch 返回false 才会执行onTouchEvent()
        ...
        if (actionMasked == MotionEvent.ACTION_UP ||
            ...
            //[+LEUI]add for LEUI-21850 by weijiabai begin
            if (AccessibilityManager.getInstance(mContext).isNiTrackerEnabled()) {
                 sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_TOUCH);
            }
            //[-LEUI]add for LEUI-21850 by weijiabai end
            stopNestedScroll();
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if ((viewFlags & ENABLED_MASK) == DISABLED) {
            ...
            //View是diable的,不响应event,但仍消费event,如果返回true
            return (((viewFlags & CLICKABLE) == CLICKABLE
                    || (viewFlags & LONG_CLICKABLE) == LONG_CLICKABLE)
                    || (viewFlags & CONTEXT_CLICKABLE) == CONTEXT_CLICKABLE);
        }
   ...
   /**
    ...
    protected void onAttachedToWindow() {
		// [+LEUI-9331]
		// only one view is allowed to apply a window level blur.
		// if multiple views requesting window blur, only the latest one works.
		checkSetBlurMode();
		// [-LEUI-9331]
    ...
    protected void onDetachedFromWindowInternal() {
        // [+LEUI-9331]
        if(mAttachInfo != null && mAttachInfo.mViewRequestingWindowBlur == this) {
            mAttachInfo.mViewRequestingWindowBlur = null;
        }
        // [-LEUI-9331]
    ...
    */
    protected BlurParams getBlurParams() {
        // TODO: use lazy loading if we need to reduce view object footprint
        return mBlurParams;
    }

    /**
    * Get a copy of the BlurParams in this view.
    * @hide
    */
    private BlurParams getBlurParamsCopy() {
        return new BlurParams(mBlurParams);
    }

    /**
    * Set the BlurParams in this view by copying bp.
    * @param bp The BlurParams to be copied to view.
    * @hide
    */
    private void setBlurParamsCopy(BlurParams bp) {
        mBlurParams.set(bp);
        invalidate(false);
    }
    ...
    * @hide
    */
    public void setBlurParamsCopyDp(BlurParams bp) {
        setBlurParamsCopy(bp);
        setBlurRadiusDp(bp.radius);
    }

    /**
    * @hide
    */
    protected float getBlurAlpha() {
        float viewAlpha = getFinalAlpha();
        return mBlurAlpha * viewAlpha;
    }

    private void checkSetBlurMode() {
        switch (getBlurParams().mode) {
        case BlurParams.BLUR_MODE_VIEW:
            break;
        case BlurParams.BLUR_MODE_WINDOW:
            if(mAttachInfo != null && mAttachInfo.mViewRequestingWindowBlur != this) {
                mAttachInfo.mViewRequestingWindowBlur = this;
            }
            break;
        case BlurParams.BLUR_MODE_NONE:
        default:
            if(mAttachInfo != null && mAttachInfo.mViewRequestingWindowBlur == this) {
                mAttachInfo.mViewRequestingWindowBlur = null;
            }
    ...
    /**
    * @param mode Blur mode of this view, May be either:
    * BlurParams.BLUR_MODE_NONE : no blur
    * BlurParams.BLUR_MODE_WINDOW : blur below this window, using the bounds of this view as blur area.
    * only one view in a window can apply window blur simultanously, you can change the active window-blur view by calling this method to override previous setting.
    * @hide
    */
    public void setBlurMode(int mode){
        getBlurParams().mode = mode;
        checkSetBlurMode();
    }

    /**
    * @param alpha Used as the weight to interpolate blur params in animation.
    * @hide
    */
    ...
    /**
    * @param radius Blur radius(actually it's sigma, same as PhotoShop) in pixel
    * @hide
    */
    public void setBlurRadius(float radius) {
        getBlurParams().radius = radius;
        invalidate(false);
    }

    /**
    * @param radiusDp Blur radius(actually it's sigma, same as PhotoShop) in dip
    * @hide
    */
    public void setBlurRadiusDp(float radiusDp) {
        float radius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, radiusDp, mResources.getDisplayMetrics());
        getBlurParams().radius = radius;
        invalidate(false);
    }

    /**
    * @param contrast Luma contrast adjustment after blur, default = 1.0.
    * @hide
    */
    public void setBlurLumaContrast(float contrast) {
        getBlurParams().lumaContrast = contrast;
        invalidate(false);
    }

    /**
    * @param brightness Luma brightness adjustment after blur, default = 0.0.
    * @hide
    */
    public void setBlurLumaBrightness(float brightness) {
        getBlurParams().lumaBrightness = brightness;
        invalidate(false);
    }

    /**
    * @param contrast Chroma contrast adjustment after blur, default = 1.0.
    * @hide
    */
    public void setBlurChromaContrast(float contrast) {
        getBlurParams().chromaContrast = contrast;
        invalidate(false);
    }

    /**
    * @param color Color of blend layer after blur, ubyte ARGB in int32
    * @hide
    */
    public void setBlurBlendColor(int color) {
        getBlurParams().blendColor = color;
        invalidate(false);
    }

    /**
    * @param presetId ID of the BlurParams preset, defined in BlurParams.PRESET_*,
    * presets are named in PRESET_<COLOR>_<SIZE> convention.
    * <COLOR> can be: DARK, NORMAL, LIGHT, LIGHTER, LIGHTEST, etc.
    * <SIZE> can be: S, M, L, XL, XXL, etc.
    * A preset contains all blur params except blur mode & area.
    * @hide
    */
    public void setBlurPreset(int presetId) {
        int mode = getBlurParams().mode;
        BlurParams bp = BlurPresets.getInstance().resolve(presetId);
        if (bp != null) {
            setBlurParamsCopyDp(bp);
            setBlurMode(mode);
        }
    }

    /**
    * @param outRect bounding box of this view in its window.
    * @hide
    */
    protected void getRectInWindow(Rect outRect) {
        if (outRect == null) outRect = new Rect();

        float[] bb = new float[4];

        if (!hasIdentityMatrix()) {
            getMatrix().mapPoints(bb);
        }
        ...
        outRect.set(Math.round(bb[0]), Math.round(bb[1]), Math.round(bb[2]), Math.round(bb[3]));
    }
    // [-LEUI-9331]
    /**
     ...
     */
    //ViewGroup循环绘制子View时调用这个3参数的draw()
    ...
    //MeasureSpec由父View得来; 不管普通View还是ViewGroup,都走这里
    public final void measure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if ((mPrivateFlags & PFLAG_FORCE_LAYOUT) == PFLAG_FORCE_LAYOUT ||
            ...
            if (cacheIndex < 0 || sIgnoreMeasureCache) {
                ...
                //如果是ViewGroup,比如frameLayout,就调用FrameLayout的onMeasure,onMeasure会循环测量子View
                //调用子View 的measure-> onMeasure
                //普通View,通过传入的MeasureSpec测量View本身,MeasureSpec由父View计算->
                //计算一般过程:specSize: 父View的宽-父View padding-子View margin-用掉的宽; specMode:根据
                //父View和子View的specMode得到
                //传入父View Spec
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        //setMeasureDimension()传入得到的view 的宽高,在此方法前要计算出View的宽高
    ...
    final static class AttachInfo {
		...
		// [+LEUI-9331]
        /**
         * Used to track which view request blur effect on window.
         * Only one view can request this in a window.
         */
        View mViewRequestingWindowBlur;
		// [-LEUI-9331]
