...
public final class SQLiteDatabase extends SQLiteClosable {
    ...
    //[+LEUI][change to wal sync off for sqlite benchmark] [chenjingjing]
    private static final String[] force_wal_apps = {
    ...
    //[+LEUI][change to wal sync off for sqlite benchmark] [chenjingjing]
    private static boolean isForceEnableWAL(String path) {
        for (String name : force_wal_apps) {
    ...
    public static SQLiteDatabase openDatabase(String path, CursorFactory factory, int flags,
            DatabaseErrorHandler errorHandler) {
        //[+LEUI][change to wal sync off for sqlite benchmark] [chenjingjing]
        boolean specialProcessingFlag = false;

        if(isForceEnableWAL(path)) {
            specialProcessingFlag = true;
            flags |= ENABLE_WRITE_AHEAD_LOGGING;
        }
        //[-LEUI]
        ...
        if (specialProcessingFlag) {
            db.rawQuery("PRAGMA synchronous=0", null);
        }
        //[-LEUI]
