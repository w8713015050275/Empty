...
import com.letv.leui.util.immersivemode.EuiImmersiveModeManager;
import com.letv.leui.fsw.FSWGuard;
//[+LEUI][LEUI-9982][guojian@letv.com] for make statusbar changeable for thidrpart app.
//import com.letv.leui.util.statusbar.*;
//[-LEUI]
/**
...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    //[+LEUI-21622]
    private boolean mEuiForcedStatusBarColor = false;
    private boolean mEuiForcedNavigationBarColor = false;//Usless now.
    EuiImmersiveModeManager mImmersiveModeManager;
    //[-LEUI-21622]
    //[+XSEVEN-9017]
    private boolean mSetLocalFocus = false;
    //[-XSEVEN-9017]
    ...
    public PhoneWindow(Context context) {
        ...
        //[+LEUI-21622]
        mImmersiveModeManager = new EuiImmersiveModeManager(this,context);
        //[-LEUI-21622]
    }
    ...
    public boolean superDispatchTouchEvent(MotionEvent event) {
        //执行DecorView的superDispatchTouchEvent
    ...
    //[+LEUI-21622]
    private final class ImmersiveView extends View{
        ...
        protected void onDraw(Canvas canvas) {
            ...
            super.onDraw(canvas);
            if(EuiImmersiveModeManager.DEBUG){
                Slog.i(TAG, "ImmersiveView onDraw package="+getContext().getPackageName());
                if(mEuiForcedStatusBarColor){
                Paint pt = new Paint();
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        public boolean dispatchTouchEvent(MotionEvent ev) {
            ...
            //从PhoneWindow传递给Activity的callback的dispatchTouchEvent
        ...
        private ActionMode startActionMode(
            ...
            if (mode != null) {
                ...
                //[+LEUI][dongshangyong]Add: Add LEUI custom action popup style.
                else if (mode.getType() == android.widget.Editor.LeTextActionModelImpl.TYPE_LE_FLOATING) {
                    if (mFloatingActionMode != null) {
                        mFloatingActionMode.finish();
                    }
                    mFloatingActionMode = mode;
        ...
        private WindowInsets updateColorViews(WindowInsets insets, boolean animate) {
            ...
            if (!mIsFloating && ActivityManager.isHighEndGfx()) {
              ...
              //  disallowAnimate |= ((mLastWindowFlags ^ attrs.flags)
               ...
               disallowAnimate |= (((mLastWindowFlags ^ attrs.flags)
                        & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0 || ((mLastLeuiFlags ^ attrs.leuiFlags) & LEUI_FLAG_TRANSLUCENT_STATUS) != 0);
                mLastLeuiFlags = attrs.leuiFlags;
        ...
        private void updateColorViewInt(final ColorViewState state, int sysUiVis, int color,
            ...
            state.present = size > 0 && (sysUiVis & state.systemUiHideFlag) == 0
                   ...
                   &&((getAttributes().flags & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0
                        || (getAttributes().leuiFlags & LEUI_FLAG_TRANSLUCENT_STATUS) != 0);
            // [-LEUI]
            ...
            if (view == null) {
                if (show) {
                    state.view = view = new ImmersiveView(mContext);//new View(mContext);
        ...
        public void updateImmersiveMode() {
            // TODO Auto-generated method stub
            //Need update status bar color and status bar icon color here.
            //[+XSEVEN-9017] don't updateImmersiveMode when setLocalFocus by app,modify for cts.
            if (mSetLocalFocus) {
                mSetLocalFocus = false;
                return;
            }
            // [-XSEVEN-9017]
            if(EuiImmersiveModeManager.ENABLE_IMMERSIVE_MODE){
                int windowFlags = getAttributes().flags;
                int euiFlags = getAttributes().leuiFlags;
                int sysUiVisibility = getAttributes().systemUiVisibility | getWindowSystemUiVisibility();
                //For immersive status bar
                boolean onlyUpdateIconColor = false;
                boolean euiImmersiveStatusBar = (euiFlags
                        & WindowManager.LayoutParams.LEUI_FLAG_TRANSLUCENT_STATUS) != 0;
                boolean needUpdate = !mForcedStatusBarColor
                        && !mIsStartingWindow && euiImmersiveStatusBar
                        && (windowFlags & WindowManager.LayoutParams.FLAG_FULLSCREEN) == 0
                        && (windowFlags & WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS) == 0
                        && (sysUiVisibility & SYSTEM_UI_FLAG_FULLSCREEN) == 0;

                if((windowFlags & WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS) != 0
                        || ((windowFlags & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0
                            &&(sysUiVisibility & SYSTEM_UI_FLAG_LIGHT_STATUS_BAR) != 0)){
                    //EUI not support translucent,so need update the status bar icon.
                    // light status bar take effect request set window flag(FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
                    onlyUpdateIconColor = true;
                    needUpdate = true;
                }
                if(EuiImmersiveModeManager.DEBUG){
                    Log.i(TAG,"updateImmersiveMode"
                        + "windowFlags=0x"+Integer.toHexString(windowFlags)
                        +" euiFlags=0x"+Integer.toHexString(euiFlags)
                        +" sysUiVisibility=0x"+Integer.toHexString(sysUiVisibility)
                        +" forceStatusBarColor="+mForcedStatusBarColor
                        +" mIsStartingWindow="+mIsStartingWindow
                        +" onlyIconColor="+onlyUpdateIconColor
                        +" needUpdate="+needUpdate
                        +" package="+mContext.getPackageName());
                }
                if(mImmersiveModeManager != null && needUpdate){
                    mImmersiveModeManager.updateImmersiveState(this, onlyUpdateIconColor, EuiImmersiveModeManager.IMMERSIVE_STATUS_BAR);
                }

                //For immersive navigation bar
                //Ignore now.
            }
        }
        //[-LEUI-21622]
        ...
        private ActionMode createActionMode(
            ...
            switch (type) {
                ...
                //[+LEUI][dongshangyong]Add: Add LEUI custom action popup style.
                case android.widget.Editor.LeTextActionModelImpl.TYPE_LE_FLOATING:
                    return createLeFloatingActionMode(originatingView, callback);
                //[-LEUI]
            }
        }
        ...
        private void setHandledActionMode(ActionMode mode) {
            ...
            //[+LEUI][dongshangyong]Add: Add LEUI custom action popup style.
                ...
                setHandledLeFloatingActionMode(mode);
            }
            //[-LEUI]
        }
        ...
        //[+LEUI][dongshangyong]Add: Add LEUI custom action popup style.
        private com.letv.leui.text.LeFloatingToolbar mLeFloatingToolbar;
        private ActionMode createLeFloatingActionMode(
                View originatingView, ActionMode.Callback2 callback) {
            if (mFloatingActionMode != null) {
                mFloatingActionMode.finish();
            }
            cleanupFloatingActionModeViews();
            final android.widget.Editor.LeTextActionModelImpl mode =
                    new android.widget.Editor.LeTextActionModelImpl(mContext, callback, originatingView);
            mFloatingActionModeOriginatingView = originatingView;
            mFloatingToolbarPreDrawListener =
                new OnPreDrawListener() {
                    @Override
        ...
        private void setHandledLeFloatingActionMode(ActionMode mode) {
            mFloatingActionMode = mode;
            mLeFloatingToolbar = new com.letv.leui.text.LeFloatingToolbar(mContext, PhoneWindow.this);
            ((android.widget.Editor.LeTextActionModelImpl) mFloatingActionMode).setFloatingToolbar(mLeFloatingToolbar);
            mFloatingActionMode.invalidate();  // Will show the floating toolbar if necessary.
            mFloatingActionModeOriginatingView.getViewTreeObserver()
                .addOnPreDrawListener(mFloatingToolbarPreDrawListener);
        }

        private void cleanupLeFloatingActionModeViews() {
            if (mLeFloatingToolbar != null) {
                mLeFloatingToolbar.dismiss();
                mLeFloatingToolbar = null;
            }
            if (mFloatingActionModeOriginatingView != null) {
                if (mFloatingToolbarPreDrawListener != null) {
                    mFloatingActionModeOriginatingView.getViewTreeObserver()
        ...
        private class ActionModeCallback2Wrapper extends ActionMode.Callback2 {
            ...
            public void onDestroyActionMode(ActionMode mode) {
                ...
                //[+LEUI][dongshangyong]Add: Add LEUI custom action popup style.
                final boolean isLeFloating = (mode.getType() ==
                        android.widget.Editor.LeTextActionModelImpl.TYPE_LE_FLOATING);
                if (isLeFloating) {
                    cleanupLeFloatingActionModeViews();
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        //LEUI-START [REQ][LEUI-1227] [zhangli] added
        int statusBarIconColor = a.getColor(R.styleable.Window_leStatusBarIconColor, Color.WHITE);
        setStatusBarIconColor(statusBarIconColor);
        //LEUI-END [REQ][LEUI-1227] [zhangli] added
        ...
        //[+LEUI-17466]
        boolean needCheckOpaqueNavigation = false;
        //[-[+LEUI-17466]]
        ...
        if (!mIsFloating && ActivityManager.isHighEndGfx()) {
            if (!targetPreL && a.getBoolean(
                  ...
                  needCheckOpaqueNavigation = true;
            }
        }
        ...
        //[+LEUI-17466]
        if(needCheckOpaqueNavigation && mNavigationBarColor == Color.BLACK &&
                a.getBoolean(R.styleable.Window_leWindowOpaqueNavigation, false)){
            addLeuiFlags(WindowManager.LayoutParams.LEUI_FLAG_OPAQUE_NAVIGATION);
        }
        //[-LEUI-17466]

        //[+LEUI-21622]
        if(EuiImmersiveModeManager.ENABLE_IMMERSIVE_MODE
                && !mIsFloating && ActivityManager.isHighEndGfx() ){
            //FIXME: window set floating but decorview can specify full screen size.
            if(!mForcedStatusBarColor && (mStatusBarColor == Color.BLACK)){
                addLeuiFlags(LEUI_FLAG_TRANSLUCENT_STATUS);
            }
        }
        //[-LEUI-21622]
        ...
        //+LEUI [REQ][LEUI-24971] [fengzihua] added: fake starting window
        int fakeRes = 0;
        if(mIsStartingWindow && mFSWGuard != null) {
            fakeRes = mFSWGuard.resolveFakeLayoutId(getContext(), mLabelRes);
            fakeWindow = (fakeRes > 0);
        }
        if(fakeWindow) {
            layoutResource =fakeRes;
        }
        //-LEUI
        ...
        View in = null;
        if(fakeWindow) {
            in = mFSWGuard.inflate(mLayoutInflater, layoutResource, null);
        } else {
            in = mLayoutInflater.inflate(layoutResource, null);
        }
        //-LEUI
        ...
        if (getContainer() == null && !fakeWindow) {
    ...
    public void setStatusBarColor(int color) {
        ...
        //mEuiForcedStatusBarColor = false;
        if (mDecor != null) {
            mDecor.updateColorViews(null, false /* animate */);
        }
        //[+CORAL-5187] when app setStatusBarColor after setEuiStatusBarColor,need to update statusbar icon color
        if(mImmersiveModeManager != null && mEuiForcedStatusBarColor){
            setStatusBarIconColor(mImmersiveModeManager.getImmersiveIconClor(color));
        }
        mEuiForcedStatusBarColor = false;
        //[-CORAL-5187]
    }
    ...
    public void setNavigationBarColor(int color) {
        ...
        mEuiForcedNavigationBarColor = false;
        //[+LEUI-17466]
        if(mNavigationBarColor != Color.BLACK){
            final int lefl = getAttributes().leuiFlags;
            if((lefl & WindowManager.LayoutParams.LEUI_FLAG_OPAQUE_NAVIGATION) != 0){
                //Clear the opaque navigation mode if app interest it.
                getAttributes().leuiFlags &= ~WindowManager.LayoutParams.LEUI_FLAG_OPAQUE_NAVIGATION;
            }
        }
        //[-LEUI-17466]
        if (mDecor != null) {
    ...
    //
    @Override
    public void setEuiStatusBarColor(int color) {
        // TODO Auto-generated method stub
        mStatusBarColor = color;
        mEuiForcedStatusBarColor = true;
        if (mDecor != null) {
            mDecor.updateColorViews(null, false /* animate */);
        }
    }
    ...
    @Override
    public void setEuiNavigationBarColor(int color) {
        // TODO Auto-generated method stub
        mNavigationBarColor = color;
        mEuiForcedNavigationBarColor = true;
        if (mDecor != null) {
            mDecor.updateColorViews(null, false /* animate */);
        }
    }
    ...
    //+LEUI [REQ][LEUI-24971] [fengzihua] added: fake starting window
    private FSWGuard mFSWGuard;
    private int mLabelRes;
    public void setFSWGuard(FSWGuard fswGuard, int labelRes) {
        mFSWGuard = fswGuard;
        mLabelRes = labelRes;
    }
    //-LEUI
